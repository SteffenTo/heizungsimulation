...
##        self.s_styropor = s_styropor
        ...
        def r_lambda(s, lambda_allgemein, flaeche):                              #allgemeine Funktion zur Berechnung eines einzelnen Wärmewiderstandes
                r_lambda = s/(lambda_allgemein * flaeche)
                return r_lambda

        def r_lambda_pzs(status_styropor, s_styropor, flaeche):                 #Berechung des Wärmewiderstandes von der oft benötigten Kombination Putz + Ziegel + Putz + evtl. Styropor
        ...
        print(r_lambda_k_wand)
        r_lambda_l_wand = ((1/r_lambda_pzs(status_styropor_wand, s_styropor, l_wand)) + (2/r_lambda(s_fenster, lambda_glas, fenster)))**(-1)
        print(r_lambda_l_wand)
        r_lambda_boden = r_lambda_pzs(status_styropor_boden, s_styropor, boden)
        print(r_lambda_boden)
        

        def r_lambda_gesamt(lambda_glas, s_fenster, s_styropor):

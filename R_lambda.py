class Waermewiderstand:
    ...
    def __init__(self, _Lambda_glas, s_fenster, status_styropor_wand, status_styropor_boden, status_styropor_dach,
                 s_styropor):  # s_fenster evtl == s_wand
        ...
        self.r_lambda_dach = ((1 / self.r_lambda_pzs(status_styropor_dach, s_styropor, self.dach)) + (
                    1 / self.r_lambda(s_fenster, _Lambda_glas, self.dachfenster))) ** (-1)
        self.r_lambda_k_wand = ((1 / self.r_lambda_pzs(status_styropor_wand, s_styropor,self. k_wand)) + (
                    1 / self.r_lambda(self.s_tuer, _Lambda_glas, self.tuer)) + (
                                            1 / self.r_lambda(s_fenster, _Lambda_glas, self.fenster))) ** (
                                   -1)  # _Lambda_glas der Tür muss immer gleich bleiben und verändert sich nicht mit der Doppelverglasung
    ...
    def r_lambda(self, s, lambda_allgemein,
                 flaeche):  # allgemeine Funktion zur Berechnung eines einzelnen Wärmewiderstandes
        r_lambda = s / (lambda_allgemein * flaeche)
        return r_lambda

    def r_lambda_pzs(self, status_styropor, s_styropor,
                     flaeche):  # Berechung des Wärmewiderstandes von der oft benötigten Kombination Putz + Ziegel + Putz + evtl. Styropor
        r_lambda_pzs = self.r_lambda(self.s_putz, self.lambda_putz, flaeche) + self.r_lambda(self.s_mauer, self.lambda_hochlochziegeln,
                                                                                        flaeche)
